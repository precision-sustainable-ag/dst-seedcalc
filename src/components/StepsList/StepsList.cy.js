...
describe('<StepsList />', () => {
  beforeEach(() => {
    cy.mount(
      <Provider store={store}>
        <StepsList availableSteps={completedList} activeStep={0} />
      </Provider>,
    );
  });
  ...
  it('should trigger the setStep function using next and back button', () => {
    ...
    cy.mount(
      <Provider store={store}>
        <StepsList activeStep={3} setActiveStep={mockSetStep} availableSteps={completedList} />
      </Provider>,
    );
  ...
  it('should show current step', () => {
    cy.mount(
      <Provider store={store}>
        <StepsList activeStep={3} availableSteps={completedList} />
      </Provider>,
    );
  ...
  it('should have restrictions on some steps', () => {
    cy.mount(
      <Provider store={store}>
        <StepsList activeStep={0} availableSteps={completedList} />
      </Provider>,
    );
    cy.get('.MuiTooltip-tooltip').invoke('text').should('equal', 'Please enter the necessary info below.');
    cy.mount(
      <Provider store={store}>
        <StepsList activeStep={1} availableSteps={completedList} />
      </Provider>,
    );
    ...
    cy.mount(
      <Provider store={store}>
        <StepsList activeStep={5} availableSteps={completedList} />
      </Provider>,
    );
    cy.getByTestId('next_button').click({ force: true });
